...
def match_pattern2(temp_gray, img_gray, bbox, angle, method, threshold):
    # height_temp,width_temp = temp_gray.shape[:2]
    ...
    rotated_tmp = cv2.warpAffine(temp_gray, rotation_matrix, (width_temp, height_temp),borderValue=(0, 0, 0), flags=cv2.INTER_LINEAR)
...
# Áp dụng biến đổi affine cho mask
    ...
    match_template(img_gray, rotated_tmp,mask,method,angle,100,threshold)
